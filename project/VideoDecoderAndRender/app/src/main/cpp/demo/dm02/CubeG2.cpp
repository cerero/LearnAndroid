...
#include "dm02/Cube.h"
#include "dm02/matrix.h"
#include "dm02/mylog.h"
#include "dm02/CubeG2.h"

void naInitGL20_dm02(JNIEnv* env, jclass clazz, jstring vertexShaderStr, jstring fragmentShaderStr);
void naDrawGraphics_dm02(JNIEnv* env, jclass clazz, float pAngleX, float pAngleY);
void naSurfaceChanged_dm02(JNIEnv* env, jclass clazz, int width, int height);

void CubeG2::OnLoad(JNIEnv* env, void* reserved)
{
	JNINativeMethod nm[3];
	nm[0].name = "naInitGL20";
	nm[0].signature = "(Ljava/lang/String;Ljava/lang/String;)V";
	nm[0].fnPtr = (void*)naInitGL20_dm02;
	nm[1].name = "naDrawGraphics";
	nm[1].signature = "(FF)V";
	nm[1].fnPtr = (void*)naDrawGraphics_dm02;
	nm[2].name = "naSurfaceChanged";
	nm[2].signature = "(II)V";
	nm[2].fnPtr = (void*)naSurfaceChanged_dm02;
	jclass cls = env->FindClass("com/example/videodecoderandrender/demo/chap04/dm02/MyRenderer");
	env->RegisterNatives(cls, nm, 3);
}


GLfloat vertices_dm02[108] = {
