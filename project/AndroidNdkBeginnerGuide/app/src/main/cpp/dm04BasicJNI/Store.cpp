...
void throwNotExistingKeyException(JNIEnv* pEnv) {
    jclass clazz = pEnv->FindClass(
            "com/example/ndkguide/dm04BasicJNI/exception/NotExistingKeyException");
    if (clazz != NULL) {
...
void throwInvalidTypeException(JNIEnv* pEnv) {
    jclass clazz = pEnv->FindClass(
            "com/example/ndkguide/dm04BasicJNI/exception/InvalidTypeException");
    if (clazz != NULL) {
...
void throwStoreFullException(JNIEnv* pEnv) {
    jclass clazz = pEnv->FindClass(
            "com/example/ndkguide/dm04BasicJNI/exception/StoreFullException");
    if (clazz != NULL) {
...
void* runWatcher(void *pArgs) {
    ...
    while (true) {
        ...
        if (!watcher->mRunning) {
            LOGW(1, "native thread stop..");
            break;
        } else {
            LOGDE(1, "native thread run..");
        }
